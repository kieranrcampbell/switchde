...
EM_sigmoid <- function(y, pst, iter = 100, log_lik_tol = 1e-3) {
  ...
  for(it in seq_len(iter)) {
    ...
    opt <- optim(params, 
                 ...
                 control = list(fnscale = -1, REPORT = TRUE, trace = 6))
    params <- opt$par
    if(abs(Q_val - opt$value) < log_lik_tol) {
      # Converged
      message(paste("Expectation-maximisation converged in", it, "iterations"))
      return(list(params = params, x = E$Ex, log_lik = opt$value))
    }
    Q_val <- opt$value
  }
  message("EM algorithm failed to converge. Consider increasing maximum iterations.")
  message("Returning most recent parameter estimates anyway")
...
test_grad <- function() {
  Q_grad_sigmoid(params2, y, pst, Ex, Ex2)
  numDeriv::grad(Q_sigmoid, params2, y = y, pst = pst, Ex = Ex, Ex2 = Ex2)
  
  Q_grad_sigmoid(params, y, pst, Ex, Ex2)
  numDeriv::grad(Q_sigmoid, params, y = y, pst = pst, Ex = Ex, Ex2 = Ex2)
}